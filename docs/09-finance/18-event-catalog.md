<!-- 
  ...
  Đây là DRAFT specification cho Event Catalog.
  
  Conflict với v3.4 architecture (Doc 01 section 7.6):
    - v3.4 plan: Kafka full event-driven trong v3.7
    - Doc 18 đề xuất: Outbox + in-process dispatcher TRƯỚC,
      Kafka chỉ khi cần scale (sau pilot)
  
  → Outbox-first là approach pragmatic hơn cho pilot phase.
  Sẽ cập nhật Doc 01 và MIGRATION-PLAN-v4 ở step sau.
  
  Tham chiếu chéo:
    - Doc 16 Finance Ledger (producer của settlement events)
    - Doc 17 Settlement Lifecycle (state machine emit events)
    - Doc 01 Architecture (sẽ update Event Bus section)
